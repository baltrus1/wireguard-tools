...
static bool kernel_has_wireguard_interface(const char *iface)
{
	const char* kernel_interface_name = "wg";
	if (strncmp(iface, kernel_interface_name, strlen(kernel_interface_name)))
		return false;
...
static int kernel_get_wireguard_interfaces(struct string_list *list)
{
	...
	while ((ent = readdir(dir))) {
		...
		if (strncmp(end, SOCK_SUFFIX, strlen(SOCK_SUFFIX)))
			continue;
		*end = '\0';
		if (!kernel_has_wireguard_interface(ent->d_name))
			continue;
